...
$type = $argv[1];		// BS,地上波を指定
$temp_data = $argv[2];	// 録画データ
$temp_xml = $argv[3];	// 番組表xml
$chkfile = $argv[4];	// 処理終了フラグ
...
if ($type == 'BS') {
	...
	if( $num == 0 ) {
	 	$cmdline = "CHANNEL=211 DURATION=180 TYPE=BS TUNER=0 MODE=0 OUTPUT=".$temp_data." ".DO_RECORD . " >/dev/null 2>&1";
  		exec( $cmdline );
  		$cmdline = $settings->epgdump." /BS ".$temp_data." ".$temp_xml;
  		exec( $cmdline );
		...
		if ($settings->cs_rec_flg != 0) {
			$cmdline = "CHANNEL=CS8 DURATION=120 TYPE=CS TUNER=0 MODE=0 OUTPUT=".$temp_data." ".DO_RECORD . " >/dev/null 2>&1";
			exec( $cmdline );
			$cmdline = $settings->epgdump." /CS ".$temp_data." ".$temp_xml;
			exec( $cmdline );
			storeProgram( "CS", $temp_xml );
			if( file_exists( $temp_data ) ) @unlink( $temp_data );
			if( file_exists( $temp_xml ) ) @unlink( $temp_xml );

			$cmdline = "CHANNEL=CS24 DURATION=120 TYPE=CS TUNER=0 MODE=0 OUTPUT=".$temp_data." ".DO_RECORD . " >/dev/null 2>&1";
			exec( $cmdline );
			$cmdline = $settings->epgdump." /CS ".$temp_data." ".$temp_xml;
			exec( $cmdline );
			storeProgram( "CS", $temp_xml );
	...
	foreach( $GR_CHANNEL_MAP as $key=>$value ){
		...
		if( $num == 0 ) {
			...
			exec( $cmdline );
			$cmdline = $settings->epgdump." ".$key." ".$temp_data." ".$temp_xml;
			exec( $cmdline );
